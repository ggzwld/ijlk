...
export default function Connect() {
  ...
  const uploadGroupAvatarToB2 = async (file: File): Promise<string | null> => {
    try {
      ...
      const response = await fetch('/.netlify/functions/upload-to-b2', {
        method: 'POST',
        body: formData,
      });
  ...
  const handleCreateGroup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (groupAvatar) {
        avatarUrl = await uploadGroupAvatarToB2(groupAvatar);
      }

      // Upload banner if provided
      if (groupBanner) {
        bannerUrl = await uploadGroupAvatarToB2(groupBanner);
      }
      ...
      const { data: newGroup, error: createError } = await supabase
        ...
        .insert([{
          ...
          rules: groupFormData.rules || null,
      ...
      if (memberError) {
        ...
        addToast('Group created but failed to add creator as member', 'warning');
      } else {
